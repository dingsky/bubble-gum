...
//公共字段
type UnionWxpCommonParams struct {
...
//公共返回字段
type UnionWxpCommonBody struct {
	XMLName xml.Name `xml:"xml"`
...
type UnionWxpResqDetail struct {
	XmlDetail      UnionDetailData  `xml:"detail,omitempty"`                    // 商品详情 XML数据
	UrlDetail      string     `xml:"-"` 			//签名数据  有效数据
}
type UnionDetailData struct{
	DetailDataString  string   `xml:",cdata"`  //<!CDATA[...]>
}

type UnionWxpPayReq struct {
	UnionWxpCommonParams
...
type UnionWxpPayResp struct {
	...
	TradeState         string `xml:"trade_state,omitempty" url:"trade_state,omitempty"`       	  //交易状态
	TradeStateDesc     string `xml:"trade_state_desc,omitempty" url:"trade_state_desc,omitempty"` //交易状态描述
}